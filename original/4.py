...
def get_pile_worths(lines : list[str]) -> list[int]:
	worths = []
	for line in lines:
...
def get_copies(lines : list[str]) -> list[int]:
	...
	for line_n, line in enumerate(lines):
		...
		winning_numbers = get_winning_numbers(line)
		if len(winning_numbers) > 0:
			worths.append((1 * pow(2, len(winning_numbers) - 1)) * copies[line_n])
...
print(sum(get_pile_worths(input_lines)))
print(sum(get_copies(input_lines)))